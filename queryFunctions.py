...
def timeInsertRowsWithCache(conn, schemaName, tableName, rowCount, dictionarySize, amountOfPossibilities, bddSize, bddCombiner = '&', name = "person"):
    cur = conn.cursor()
    ...
    cur.execute("BEGIN;")
    cur.execute(cur.mogrify('explain analyze ' + insert_data_query))
    analyze_fetched = cur.fetchall()
    cur.execute("ROLLBACK;")
    executionTime = float(analyze_fetched[-1][0][16:21])
    cur.close()
...
def timeInsertRows(conn, schemaName, tableName, rowCount, dictionarySize, amountOfPossibilities, bddSize, bddCombiner = '&'):
    cur = conn.cursor()
